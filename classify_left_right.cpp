...
private:
                            ...
                            const sensor_msgs::msg::PointCloud::SharedPtr& msg)
    {
        ...
        frame_id_marker.pose.position.x = 0;
        frame_id_marker.pose.position.y = 0;
        frame_id_marker.pose.position.z = 5.0;
        frame_id_marker.scale.z = 0.8;
        ...
        for (const auto& indices : cluster_indices)
        {
            ...
            // --- MODIFICATION: DETERMINE COLOR BASED ON Y-COORDINATE ---
            // Determine color code: 1 for Blue (left side, y > 0), 0 for Yellow (right side, y < 0)
            int color_marker_value = (centroid[1] > 0) ? 1 : 0;
            ...
            // --- MODIFICATION: SET COLOR BASED ON POSITION ---
            cylinder_marker.color.a = 1.0;
            if (color_marker_value == 1) { // Corresponds to centroid[1] > 0
                // Left side = Blue
            ...
            // --- MODIFICATION: UPDATE TEXT TO SHOW Y-COORDINATE ---
            std::stringstream label_stream;
            label_stream.precision(2);
            label_stream << std::fixed << "C:" << cluster_id << "\nP:" << num_points << "\nY:" << centroid[1];
