...
public:
    DataExtractorWithMarkers() : Node("data_extractor_with_markers")
    {
        ...
        this->declare_parameter<double>("ground_height_threshold", -0.1627); // For "threshold" method
        ...
        if (!csv_file_.is_open()) {
            ...
            // --- MODIFICATION: ADDED 'color_marker' COLUMN TO CSV HEADER ---
...
private:
    ...
    void process_cloud(const sensor_msgs::msg::PointCloud::SharedPtr msg)
    {
        ...
        // Fix the line below by adding std::
        std::cout << "Number of non ground points are: " << non_ground_cloud->points.size() << std::endl;
                            ...
                            const sensor_msgs::msg::PointCloud::SharedPtr& msg)
    {
        ...
        for (const auto& indices : cluster_indices)
        {
            ...
            // --- MODIFICATION: CALCULATE AVERAGE INTENSITY AND DETERMINE COLOR ---
            ...
            // Determine color code: 1 for Blue (high intensity), 0 for Yellow (low intensity)
            int color_marker_value = (average_intensity > 1e6) ? 1 : 0;

            // Log every point in the cluster to the CSV buffer with the determined color code
            for (int index : indices.indices) {
                const auto& point = non_ground_cloud->points[index];
                std::stringstream ss;
                ss << cluster_id << "," << frame_id_ << "," << point.z << "," << point.intensity << "," << color_marker_value;
                csv_buffer_.push_back(ss.str());
            }
            
            // Add a blank line to the buffer after each cluster's data
            ...
            // --- MODIFICATION: SET COLOR BASED ON INTENSITY ---
            cylinder_marker.color.a = 1.0;
            if (color_marker_value == 1) { // Corresponds to average_intensity > 1e6
                // High intensity = Blue
            ...
            // --- MODIFICATION: UPDATE TEXT TO SHOW INTENSITY ---
            text_marker.text = "C:" + std::to_string(cluster_id) + "\nP:" + std::to_string(num_points) + "\nI:" + std::to_string(static_cast<int>(average_intensity));
            text_marker.scale.z = 0.4;
            // White text color for better readability on blue/yellow backgrounds
