...
public:
    DataExtractorWithMarkers() : Node("data_extractor_with_markers")
    {
        ...
        // --- MODIFICATION: Open CSV file with new header for point-level data ---
        csv_file_.open("point_level_data.csv");
        if (!csv_file_.is_open()) {
            RCLCPP_ERROR(this->get_logger(), "Failed to open CSV file!");
        } else {
            csv_file_ << "cluster_id,frame_id,z_coordinate,intensity\n";
            csv_file_.flush();
        }
...
private:
                            ...
                            const sensor_msgs::msg::PointCloud::SharedPtr& msg)
    {
        ...
        for (const auto& indices : cluster_indices)
        {
            ...
            // --- MODIFICATION: Log every point in the cluster to the CSV buffer ---
            {
                std::lock_guard<std::mutex> csv_lock(csv_mutex_);
                for (int index : indices.indices) {
                    const auto& point = non_ground_cloud->points[index];
                    std::stringstream ss;
                    // Format: cluster_id,frame_id,z_coordinate,intensity
                    ss << cluster_id << "," << frame_id_ << "," << point.z << "," << point.intensity;
                    csv_buffer_.push_back(ss.str());
                }
            }

            // --- Visualization logic remains the same ---
            // It still needs centroid and bounding box for markers.
            ...
            text_marker.text = "C:" + std::to_string(cluster_id) + " P:" + std::to_string(num_points);
        ...
        if (csv_buffer_.size() > 1000) { // Increased buffer size as it will fill up faster
            std::lock_guard<std::mutex> csv_lock(csv_mutex_);
            for (const auto& row : csv_buffer_) {
                csv_file_ << row << "\n";
            }
